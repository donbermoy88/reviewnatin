-- Add Special Needs Education as a LET Elementary Area of Specialization.
...
SELECT major_sa.id, 'special-needs-education', 'Special Needs Education', 2
FROM major_sa
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE et.slug = 'let-elementary'
      AND sa.slug = 'major'
      AND t.slug = 'special-needs-education'
  ) THEN
    RAISE EXCEPTION 'LET Elementary specialization topic special-needs-education was not created';
  END IF;
END $$;
