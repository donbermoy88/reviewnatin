-- Preserve the release-blocker usage-limit behavior after AI Tutor removal.
-- AI Tutor remains removed; this restores the mobile-facing mock preview field
-- and keeps timer-expired mini mocks with no submitted answers from consuming
-- the free weekly mini-mock slot.
...
BEGIN
  ...
  WHERE slug = p_exam_slug AND is_active
  LIMIT 1;
