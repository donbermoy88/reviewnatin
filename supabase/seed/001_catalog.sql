-- ReviewNatin Phase 1 catalog seed
-- Run AFTER supabase/migrations/001_mvp_schema.sql
...
INSERT INTO subscription_products (sku, tier, exam_type_id, price_php, duration_days, store) VALUES
  ('com.reviewnatin.exampass.cse_pro', 'exam_pass', 'b0000001-0001-4000-8000-000000000001', 499, 180, 'both'),
  ('com.reviewnatin.exampass.cse_sub', 'exam_pass', 'b0000001-0001-4000-8000-000000000002', 499, 180, 'both'),
  ('com.reviewnatin.exampass.let_elem', 'exam_pass', 'b0000001-0001-4000-8000-000000000003', 499, 180, 'both'),
  ('com.reviewnatin.exampass.let_sec', 'exam_pass', 'b0000001-0001-4000-8000-000000000004', 599, 180, 'both'),
  ('com.reviewnatin.exampass.pnle', 'exam_pass', 'b0000001-0001-4000-8000-000000000005', 599, 180, 'both'),
  ('com.reviewnatin.plus.monthly', 'plus', NULL, 149, 30, 'both'),
  ('com.reviewnatin.plus.yearly', 'plus', NULL, 999, 365, 'both')
ON CONFLICT (sku) DO NOTHING;

-- Sample published question (demo)
...
WHERE et.slug = 'cse-professional' AND sa.slug = 'general-info'
LIMIT 1;
