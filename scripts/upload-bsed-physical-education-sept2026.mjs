...
const sourceFile = 'output/pdf/bsed_physical_education_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_physical_education_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED Physical Ed P\d+\.pdf)/)?.[1];
...
function questionPayload(row, topicId) {
  const detailSlug = slugify(row.Topic || 'physical-education');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED Physical Education PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_physical_education_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
    ...
    major: 'MAPEH',
    major_slug: 'mapeh',
    sub_tag: row.Topic || 'Physical Education',
...
if (!topic?.id) throw new Error('Missing LET Secondary major/mapeh topic');
...
if (!dryRun) {
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_physical_education_sept2026_import',
...
const report = {
  ...
  existing_mapeh_questions_before: existing.length,
