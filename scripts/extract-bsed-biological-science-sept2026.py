...
SOURCE_DIR = Path("/Users/lyndon/All reviewers/Sept2026_LET Reviewer Files/L. BSED - Biological Science-")
OUT_DIR = ROOT / "output/pdf"
BASE = "bsed_biological_science_sept2026_extracted_questions"
...
EXPECTED_COUNTS = {
    "BSED Biological Science P1.pdf": 50,
    "BSED Biological Science P2.pdf": 50,
    "BSED Biological Science P3.pdf": 50,
    "BSED Biological Science P4.pdf": 50,
    "BSED Biological Science P5.pdf": 50,
    "BSED Biological Science P6.pdf": 50,
    "BSED Biological Science P7.pdf": 50,
    "BSED Biological Science P8.pdf": 49,
    "BSED Biological Science P9.pdf": 50,
    "BSED Biological Science P10.pdf": 46,
}
...
def words(value):
    stop = {
        ...
        "biology", "biological", "science", "which", "what", "when", "where", "called",
        "following", "organism", "organisms", "cell", "cells",
    }
...
def clean_pdf_text(text):
    ...
    for line in str(text or "").replace("\x0c", "\n").splitlines():
        ...
        if re.fullmatch(r"By:\s*Professionals Review PH", stripped, re.I):
            continue
        if re.fullmatch(r"BIOLOGICAL SCIENCE\s*\(SECONDARY EDUCATION\)", stripped, re.I):
            continue
        if re.fullmatch(r"Secondary Education-?\s*Biological Science", stripped, re.I):
            continue
        if re.fullmatch(r"BIOLOGICAL SCIENCE", stripped, re.I):
            continue
        if re.fullmatch(r"Get more Free LET Reviewers @ www\.teachpinas\.com", stripped, re.I):
            continue
...
def pdf_text(path):
    ...
    text = normalize_inline_question_markers(text)
...
def page_texts(path):
    ...
    texts = [normalize_inline_question_markers(text) for text in texts]
    return texts


def normalize_inline_question_markers(text):
    starts = (
        "A", "An", "All", "Animals", "Ant", "At", "Both", "Chlorophyll", "During",
        "Each", "How", "If", "In", "Meiosis", "One", "The", "There", "Two",
        "Unlike", "Upon", "What", "When", "Where", "Which", "Why",
    )
    starter = "|".join(re.escape(item) for item in starts)
    text = re.sub(r"(?m)^(\d{1,3})\s+\.", r"\1.", text)
    return re.sub(
        rf"(?<=[A-Za-z0-9\)])\s+([1-9]\d{{0,2}})\s*\.\s*(?=(?:{starter})\b)",
        r"\n\1. ",
        text,
    )
...
def parse_answer_key(key_text):
    ...
    for raw_line in key_text.splitlines():
        ...
        letter_pairs = list(re.finditer(r"([1-9]\d{0,2})\s*\.\s*([A-Ha-h])\b", line))
        if len(letter_pairs) > 1:
            for pair in letter_pairs:
                answers[int(pair.group(1))] = {
                    "answer_text": pair.group(2).upper(),
                    "rationale": "",
                }
            continue
...
def infer_topic(question):
    ...
    if any(x in text for x in ["dna", "rna", "gene", "genetic", "chromosome", "inheritance", "mendel", "allele", "mutation", "meiosis", "mitosis", "crossing over", "phenotype", "genotype"]):
        return "Genetics and Heredity"
    if any(x in text for x in ["organelle", "cell", "membrane", "ribosome", "mitochond", "chloroplast", "lysosome", "cytoplasm", "nucleus", "protein synthesis"]):
        return "Cell Biology and Biochemistry"
    if any(x in text for x in ["evolution", "natural selection", "species", "taxonomy", "classification", "linnaeus", "fossil", "adaptive radiation", "homologous"]):
        return "Evolution and Classification"
    if any(x in text for x in ["ecosystem", "biome", "biodiversity", "pollution", "greenhouse", "ozone", "acid rain", "deforestation", "sustainable", "trophic", "mutualism", "parasitism", "commensalism", "eutrophication"]):
        return "Ecology and Environment"
    if any(x in text for x in ["kidney", "blood", "heart", "brain", "lung", "digest", "hormone", "gland", "nervous", "bone", "muscle", "respiration", "urinary", "reproductive", "tissue"]):
        return "Anatomy and Physiology"
    if any(x in text for x in ["bacteria", "virus", "microorganism", "protozo", "fungi", "fermentation", "malaria", "tuberculosis", "vaccine", "antibiotic", "pasteur"]):
        return "Microbiology and Immunology"
    if any(x in text for x in ["plant", "flower", "seed", "root", "stem", "leaf", "xylem", "phloem", "photosynthesis", "pollination", "stomata", "chlorophyll", "fruit"]):
        return "Botany"
    if any(x in text for x in ["animal", "zoology", "fish", "mammal", "reptile", "amphibian", "arthropod", "mollusc", "primate", "metamorphosis", "ascaris"]):
        return "Zoology"
    if any(x in text for x in ["inquiry", "teaching", "laboratory", "safety", "experiment", "hypothesis", "observation", "5e", "science learning", "control variable"]):
        return "Biology Pedagogy and Research"
...
def extract_pdf(path, start_no):
    ...
    for qnum, block in split_numbered_items(question_text):
        ...
        notes = "Path and PDF header indicate BSED/Secondary Biological Science major."
        if has_answer_key:
...
def write_outputs(rows):
    ...
    summary = {
        ...
        "subject_category_breakdown": {"Biological Science": len(rows)},
        ...
        "suggested_supabase_mapping": {
            ...
            "subject_area": "Biological Science",
        ...
        "recommended_next_action": "Upload Ready for Upload rows to LET Secondary major/biological-science, then verify live source counts and sub-tag breakdown.",
...
def main():
    rows = []
    next_no = 1
    for path in sorted(SOURCE_DIR.glob("*.pdf"), key=natural_sort_key):
        if path.name in SKIPPED_SOURCES:
            continue
        if path.name not in EXPECTED_COUNTS:
            raise RuntimeError(f"Unexpected PDF in Biological Science import folder: {path.name}")
