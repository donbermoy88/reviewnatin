...
SOURCE_DIR = Path("/Users/lyndon/All reviewers/Sept2026_LET Reviewer Files/L. BSED - TLEd Agriculture and Fishery Arts-")
OUT_DIR = ROOT / "output/pdf"
BASE = "bsed_tled_afa_sept2026_extracted_questions"
...
def words(value):
    stop = {
        ...
        "used", "uses", "agriculture", "fishery", "arts", "farm", "farmer", "fish",
        "plant", "plants", "animal", "animals", "production", "process", "system",
    }
...
def clean_pdf_text(text):
    ...
    for line in str(text or "").replace("\x0c", "\n").splitlines():
        ...
        if re.fullmatch(r"By:\s*Professionals Review PH", stripped, re.I):
            continue
        if re.fullmatch(r"TLEd-?\s*Agriculture\s*and\s*Fishery Arts\s*2026?", stripped, re.I):
            continue
        if re.fullmatch(r"LET\s*Reviewer\s*TLEd\s*[-–]\s*Agricultureand Fishery Arts\s*2026?", stripped, re.I):
            continue
...
def infer_topic(question):
    ...
    if any(x in text for x in ["research", "extension", "farmer training", "communication", "dissemination", "hypothesis", "variable", "sampling", "literature", "methodology", "replicability", "survey"]):
        return "Agricultural Extension and Research"
    if any(x in text for x in ["business", "agribusiness", "market", "marketing", "budget", "profit", "credit", "cooperative", "liquidity", "record", "cash", "entrepreneur", "farm management", "planning", "income", "expense", "feasibility", "roi", "sales"]):
        return "Agribusiness and Farm Management"
    if any(x in text for x in ["tractor", "mechanization", "machine", "engine", "diesel", "harvester", "reaper", "thresher", "moldboard", "plow", "harrow", "sprinkler", "irrigation system", "pto", "rain gauge", "fuel", "transmission"]):
        return "Farm Tools, Equipment, and Mechanization"
    if any(x in text for x in ["fish", "aquaculture", "pond", "tilapia", "milkfish", "bangus", "catfish", "fingerling", "fry", "gill", "spawning", "bfAR".lower(), "fisher", "seining", "cage culture", "water quality", "dissolved oxygen", "red tide", "shrimp", "hatchery"]):
        return "Aquaculture and Fishery Arts"
    if any(x in text for x in ["postharvest", "drying", "winnowing", "threshing", "milling", "grain", "seed treatment", "packaging", "packing", "grading", "moisture", "storage", "processing", "preservation", "smoking", "salting", "freezing", "icing", "curing", "haccp", "iso", "evisceration", "fish meal"]):
        return "Postharvest and Food Processing"
    if any(x in text for x in ["cattle", "swine", "goat", "poultry", "chicken", "broiler", "layer", "sow", "boar", "calf", "cow", "castration", "weaning", "brooding", "incubation", "feed", "ration", "animal", "livestock", "rumen", "newcastle"]):
        return "Animal Production"
    if any(x in text for x in ["crop", "soil", "leaf", "root", "flower", "seed", "plant", "land preparation", "weeding", "fertilizer", "nitrogen", "phosphorus", "potassium", "irrigation", "grafting", "cutting", "pruning", "pest", "rice", "mungbean", "loam", "compost", "hydroponics", "photosynthesis", "transplanting"]):
