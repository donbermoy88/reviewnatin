...
const sourceFile = 'output/pdf/bsed_filipino_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_filipino_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED Filipino Part\d+\.pdf)/)?.[1];
...
function questionPayload(row, topicId) {
  const detailSlug = slugify(row.Topic || 'filipino');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED Filipino PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_filipino_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
...
if (!topic?.id) throw new Error('Missing LET Secondary major/filipino topic');
...
const skipped = rows
  ...
  .map((row) => ({
    ...
    image_reference: row['Question Image Reference'] || '',
    notes: row.Notes || '',
  }));
...
if (!dryRun) {
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_filipino_sept2026_import',
