...
const sourceFile = 'output/pdf/bsed_social_studies_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_social_studies_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED Social Studies-Science P\d+\.pdf)/)?.[1];
...
function questionPayload(row, topicId) {
  const detailSlug = slugify(row.Topic || 'social-studies-araling-panlipunan');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED Social Studies PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_social_studies_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
    ...
    tags: [
      ...
      'sept2026',
      'let-secondary',
      'major:social-studies-araling-panlipunan',
      'social-studies',
      'araling-panlipunan',
      'bsed-social-studies',
      `topic:${detailSlug}`,
    ...
    major: 'Social Studies / Araling Panlipunan',
    major_slug: 'social-studies-araling-panlipunan',
    sub_tag: row.Topic || 'Social Studies / Araling Panlipunan',
...
const { data: topic, error: topicError } = await sb
  ...
  .eq('slug', 'social-studies-araling-panlipunan')
  .eq('subject_areas.slug', 'major')
  .eq('subject_areas.exam_types.slug', 'let-secondary')
  .maybeSingle();
if (topicError) throw topicError;
if (!topic?.id) throw new Error('Missing LET Secondary major/social-studies-araling-panlipunan topic');
...
if (!dryRun) {
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_social_studies_sept2026_import',
...
const report = {
  ...
  existing_social_studies_questions_before: existing.length,
