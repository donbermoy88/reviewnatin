...
const sourceFile = 'output/pdf/bsed_values_education_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_values_education_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED Values Ed P\d+\.pdf)/)?.[1];
...
function questionPayload(row, topicId) {
  const topicSlug = slugify(row.Topic || 'values-education');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED Values Education PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_values_education_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
    ...
    major: 'Values Education',
    major_slug: 'values-education',
    sub_tag: row.Topic || null,
    sub_tag_slug: topicSlug,
...
const { data: topic, error: topicError } = await sb
  ...
  .eq('slug', 'values-education')
  .eq('subject_areas.slug', 'major')
  .eq('subject_areas.exam_types.slug', 'let-secondary')
  .maybeSingle();
if (topicError) throw topicError;
if (!topic?.id) throw new Error('Missing LET Secondary major/values-education topic');

async function fetchExistingValuesQuestions() {
  const all = [];
  const pageSize = 1000;
...
const existing = await fetchExistingValuesQuestions();
...
if (!dryRun) {
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_values_education_sept2026_import',
...
const report = {
  ...
  existing_values_education_questions_before: existing.length,
