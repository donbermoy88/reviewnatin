...
const sourceFile = 'output/pdf/bsed_biological_science_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_biological_science_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED Biological Science P\d+\.pdf)/)?.[1];
...
function questionPayload(row, topicId) {
  const detailSlug = slugify(row.Topic || 'biological-science');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED Biological Science PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_biological_science_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
    ...
    major: 'Biological Science',
    major_slug: 'biological-science',
    sub_tag: row.Topic || 'Biological Science',
...
const { data: topic, error: topicError } = await sb
  ...
  .eq('slug', 'biological-science')
  .eq('subject_areas.slug', 'major')
  .eq('subject_areas.exam_types.slug', 'let-secondary')
  .maybeSingle();
if (topicError) throw topicError;
if (!topic?.id) throw new Error('Missing LET Secondary major/biological-science topic');
...
const updates = [];
const duplicates = [];
const contentKeyUpdates = [];
let duplicateSourceArchives = [];
...
for (const row of readyRows) {
  ...
  if (existingByContent) {
    updates.push({ id: existingByContent.id, sourceKey, row, payload });
    contentKeyUpdates.push({
      row: row['No.'],
      source_pdf: row['Source PDF'],
      original_no: row['Original No.'],
      reason: 'adopt_existing_content',
      existing_id: existingByContent.id,
    });
    plannedByContent.set(contentKey, { id: existingByContent.id });
    continue;
  }
...
const updatedIdsPlanned = new Set(updates.map((update) => update.id));
duplicateSourceArchives = duplicateSourceArchives.filter((duplicate) => !updatedIdsPlanned.has(duplicate.id));
...
if (!dryRun) {
  ...
  for (const duplicate of duplicateSourceArchives) {
    const { error } = await sb
      ...
      .update({
        status: 'archived',
        needs_review: true,
        review_reason: `Duplicate of extracted Biological Science row ${duplicate.duplicateOf || 'unknown'}.`,
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_biological_science_sept2026_import',
      entity_type: 'questions',
      metadata: {
        inserted: insertedIds.length,
        updated: updatedIds.length,
        archived_duplicate_sources: duplicateSourceArchives.length,
        content_key_updates: contentKeyUpdates.length,
...
const report = {
  ...
  existing_biological_science_questions_before: existing.length,
  ...
  updatable_by_content_key: contentKeyUpdates.length,
  inserted: insertedIds.length,
  updated: updatedIds.length,
  ...
  content_update_samples: contentKeyUpdates.slice(0, 50),
