...
const sourceFile = 'output/pdf/bsed_tled_industrial_arts_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_tled_industrial_arts_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED TLEd-IA P\d+\.pdf)/)?.[1];
...
function candidateContentKey(row) {
  const choiceKey = ['A', 'B', 'C', 'D']
    .map((letter) => `${letter.toLowerCase()}:${normalizeText(row[`Choice ${letter}`])}`)
    .join('|');
...
function questionPayload(row, topicId) {
  const detailSlug = slugify(row.Topic || 'industrial-arts');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED TLEd Industrial Arts PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_tled_industrial_arts_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
    ...
    tags: [
      ...
      'bsed-tled-industrial-arts',
      'subtag:industrial-arts',
      `topic:${detailSlug}`,
    ],
    major: 'Technology and Livelihood Education (TLE)',
    major_slug: 'tle',
    sub_tag: row.Topic || 'Industrial Arts',
...
if (!topic?.id) throw new Error('Missing LET Secondary major/tle topic');

async function fetchExistingTleQuestions() {
  const all = [];
  const pageSize = 1000;
...
const existing = await fetchExistingTleQuestions();
...
if (!dryRun) {
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_tled_industrial_arts_sept2026_import',
...
const report = {
  ...
  existing_tle_questions_before: existing.length,
