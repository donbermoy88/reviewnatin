...
const sourceFile = 'output/pdf/bsed_tvted_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_tvted_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED TVTED P\d+\.pdf)/)?.[1];
...
function questionPayload(row, topicId) {
  const detailSlug = slugify(row.Topic || 'technology-vocational-teacher-education');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED TVTED PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_tvted_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
    ...
    major_slug: 'tle',
    sub_tag: row.Topic || 'Technology-Vocational Teacher Education',
...
if (!dryRun) {
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_tvted_sept2026_import',
