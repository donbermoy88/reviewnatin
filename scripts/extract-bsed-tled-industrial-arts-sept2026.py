...
SOURCE_DIR = Path("/Users/lyndon/All reviewers/Sept2026_LET Reviewer Files/L. BSED - TLEd Industrial Arts-")
OUT_DIR = ROOT / "output/pdf"
BASE = "bsed_tled_industrial_arts_sept2026_extracted_questions"
...
def words(value):
    stop = {
        ...
        "used", "uses", "industrial", "arts", "technology", "education", "student",
        "students", "teaching", "learning", "workshop", "tool", "tools", "purpose",
    }
...
def clean_pdf_text(text):
    ...
    for line in str(text or "").replace("\x0c", "\n").splitlines():
        ...
        if re.fullmatch(r"By:\s*Professionals Review PH", stripped, re.I):
            continue
        if re.fullmatch(r"TLEd-?\s*Industrial Arts\s*2026?", stripped, re.I):
            continue
        if re.fullmatch(r"LET\s*Reviewer\s*TLEd\s*[–-]\s*Industrial Arts\s*2026?", stripped, re.I):
            continue
...
def infer_topic(question):
    ...
    if any(x in text for x in ["teacher", "teaching", "learning", "lesson", "assessment", "rubric", "checklist", "project-based", "project method", "competency", "obe", "demonstration", "evaluation", "job sheet", "dual training", "tLE".lower()]):
        return "TLE Teaching and Assessment"
    if any(x in text for x in ["safety", "hazard", "accident", "ppe", "first aid", "fire", "extinguisher", "signage", "ventilation", "layout", "supervision", "inventory", "maintenance", "ergonomics", "workshop", "waste", "5s", "4s"]):
        return "Workshop Safety and Organization"
    if any(x in text for x in ["drawing", "drafting", "cad", "cadd", "orthographic", "isometric", "projection", "line", "scale", "dimension", "blueprint", "title block", "t-square", "compass", "protractor", "hidden line", "plan view", "schematic"]):
        return "Drafting and CAD"
    if any(x in text for x in ["current", "voltage", "resistance", "ohm", "ampere", "watt", "circuit", "wire", "fuse", "breaker", "ground", "diode", "capacitor", "transformer", "motor", "generator", "alternator", "ammeter", "voltmeter", "multimeter", "electrical", "conductor", "insulator"]):
        return "Electrical and Electronics"
    if any(x in text for x in ["car", "engine", "automotive", "battery", "carburetor", "brake", "transmission", "clutch", "crankshaft", "radiator", "spark plug", "piston", "tire", "fuel", "vehicle", "compression", "tachometer", "differential"]):
        return "Automotive Technology"
    if any(x in text for x in ["weld", "solder", "braz", "metal", "lathe", "anneal", "forging", "steel", "file", "thread", "tap", "die", "grinding", "buffing", "galvanizing", "electroplating", "sheet", "vise", "anvil", "flux", "electrode"]):
        return "Metalwork and Welding"
    if any(x in text for x in ["wood", "carpentry", "lumber", "saw", "plane", "router", "varnish", "sanding", "dovetail", "mortise", "tenon", "furniture", "plywood", "jointer", "chisel", "mahogany", "narra"]):
        return "Woodworking"
    if any(x in text for x in ["production", "quality", "tolerance", "template", "fabrication", "project plan", "bill of materials", "bom", "working drawing", "mass", "batch", "process"]):
