...
const sourceFile = 'output/pdf/bsed_tled_afa_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_tled_afa_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED TLEd-AFA P\d+\.pdf)/)?.[1];
...
function questionPayload(row, topicId) {
  const detailSlug = slugify(row.Topic || 'agriculture-and-fishery-arts');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED TLEd Agriculture and Fishery Arts PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_tled_afa_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
    ...
    tags: [
      ...
      'bsed-tled-afa',
      'subtag:agriculture-and-fishery-arts',
      `topic:${detailSlug}`,
    ],
    major: 'Technology and Livelihood Education (TLE)',
    major_slug: 'tle',
    sub_tag: row.Topic || 'Agriculture and Fishery Arts',
...
if (!dryRun) {
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_tled_afa_sept2026_import',
