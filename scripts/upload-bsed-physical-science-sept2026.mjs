...
const sourceFile = 'output/pdf/bsed_physical_science_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_physical_science_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED Physical Science (?:P\d+|[A-Z])\.pdf)/)?.[1];
...
function questionPayload(row, topicId) {
  const detailSlug = slugify(row.Topic || 'physical-science');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED Physical Science PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_physical_science_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
    ...
    major: 'Physical Science',
    major_slug: 'physical-science',
    sub_tag: row.Topic || 'Physical Science',
...
const { data: topic, error: topicError } = await sb
  ...
  .eq('slug', 'physical-science')
  .eq('subject_areas.slug', 'major')
  .eq('subject_areas.exam_types.slug', 'let-secondary')
  .maybeSingle();
if (topicError) throw topicError;
if (!topic?.id) throw new Error('Missing LET Secondary major/physical-science topic');
...
if (!dryRun) {
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_physical_science_sept2026_import',
...
const report = {
  ...
  existing_physical_science_questions_before: existing.length,
