...
const sourceFile = 'output/pdf/bsed_english_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_english_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/((?:BSED English Part\d+|SET\d Drill&Booster English)\.pdf)/)?.[1];
...
function questionPayload(row, topicId) {
  const detailSlug = slugify(row.Topic || 'english');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED English PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_english_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
...
if (!topic?.id) throw new Error('Missing LET Secondary major/english topic');
...
if (!dryRun) {
  ...
  for (const duplicate of duplicateSourceArchives) {
    const { error } = await sb
      ...
      .update({
        status: 'archived',
        needs_review: true,
        review_reason: `Duplicate of extracted English row ${duplicate.duplicateOf || 'unknown'}.`,
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_english_sept2026_import',
...
const report = {
  ...
  existing_english_questions_before: existing.length,
