...
const sourceFile = 'output/pdf/bsed_general_science_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_general_science_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED General Science (?:P\d+|[A-Z])\.pdf)/)?.[1];
...
function questionPayload(row, topicId) {
  const detailSlug = slugify(row.Topic || 'general-science');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED General Science PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_general_science_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
    ...
    sub_tag: row.Topic || 'General Science',
...
const { data: topic, error: topicError } = await sb
  ...
  .eq('slug', 'general-science')
  .eq('subject_areas.slug', 'major')
  .eq('subject_areas.exam_types.slug', 'let-secondary')
  .maybeSingle();
if (topicError) throw topicError;
if (!topic?.id) throw new Error('Missing LET Secondary major/general-science topic');
...
if (!dryRun) {
  ...
  for (const duplicate of duplicateSourceArchives) {
    const { error } = await sb
      ...
      .update({
        status: 'archived',
        needs_review: true,
        review_reason: `Duplicate of extracted General Science row ${duplicate.duplicateOf || 'unknown'}.`,
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_general_science_sept2026_import',
...
const report = {
  ...
  existing_general_science_questions_before: existing.length,
