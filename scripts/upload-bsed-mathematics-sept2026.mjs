...
const sourceFile = 'output/pdf/bsed_mathematics_sept2026_extracted_questions.json';
const reportFile = 'output/pdf/bsed_mathematics_sept2026_upload_report.json';
...
function sourceKeyFromNote(note) {
  const sourcePdf = String(note ?? '').match(/(BSED Mathematics P\d+\.pdf)/)?.[1];
...
function candidateContentKey(row) {
  const choiceKey = ['A', 'B', 'C', 'D']
    .map((letter) => `${letter.toLowerCase()}:${normalizeChoiceText(row[`Choice ${letter}`])}`)
    .join('|');
...
function questionPayload(row, topicId) {
  const topicSlug = slugify(row.Topic || 'mathematics');
  return {
    topic_id: topicId,
    stem: row.Question,
    ...
    explanation_en:
      ...
      `Answer key from extracted BSED Mathematics PDF: ${row['Correct Answer']}.`,
    ...
    source_note: [
      `bsed_mathematics_sept2026 row ${row['No.']}`,
      `page ${row.Page}`,
      row['Source PDF'],
...
if (!topic?.id) throw new Error('Missing LET Secondary major/mathematics topic');

async function fetchExistingMathQuestions() {
  const all = [];
  const pageSize = 1000;
...
const existing = await fetchExistingMathQuestions();
...
if (!dryRun) {
  ...
  if (adminUser?.id && (insertedIds.length || updatedIds.length)) {
    await sb.from('admin_logs').insert({
      ...
      action: 'bsed_mathematics_sept2026_import',
...
const report = {
  ...
  existing_math_questions_before: existing.length,
