...
export default function BookmarksScreen() {
  ...
  if (!user) {
    return (
      <StackShell title="Bookmarks" subtitle="Mga na-save na tanong at lessons mo">
        <EmptyState
  ...
  const ListHeader = (
    <>
      ...
      {!loading && tab === 'questions' && questions.length > 0 ? (
        <PrimaryButton
  ...
  const renderQuestion = (item: BookmarkItem) => (
    <View style={styles.card}>
      ...
      <View style={{ flexDirection: 'row', gap: spacing.sm, marginTop: spacing.sm }}>
        ...
        />
        <ReportContentButton contentType="question" contentId={item.questionId} label="Flag" compact />
  ...
  const renderMaterial = (item: MaterialBookmarkItem) => (
    <View style={styles.card}>
      <Text style={styles.cardSubject}>
      ...
      <View style={{ flexDirection: 'row', gap: spacing.sm, marginTop: spacing.sm }}>
        ...
        />
        <ReportContentButton contentType="review_material" contentId={item.materialId} label="Flag" compact />
  ...
  const data: (BookmarkItem | MaterialBookmarkItem)[] = loading
    ? []
    : tab === 'questions'
      ? questions
      : materials;
  ...
  return (
    <View style={styles.root}>
      <FlatList
        ...
        extraData={tab}
        keyExtractor={(item) =>
          'questionId' in item ? item.questionId : item.materialId
        }
        ...
        renderItem={({ item }) =>
          'questionId' in item ? renderQuestion(item) : renderMaterial(item)
        }
        refreshControl={
          <RefreshControl
